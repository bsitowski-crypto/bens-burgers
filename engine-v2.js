...
const IMAGES={};
function loadImages(){return Promise.all(Object.entries(ASSETS).map(([k,src])=>new Promise(res=>{const im=new Image();im.onload=()=>{IMAGES[k]=im;res()};im.onerror=()=>{IMAGES[k]=null;res()};im.src=src+'?canvas=2'})))}
...
const CATALOG={
 ...
 bottomBun:{id:'bottomBun',label:'BUNS',station:'base',asset:'bottomBun',stack:{w:190,h:62,overlap:0}}
};
const RECIPES={
...
const state={money:0,score:0,served:0,patience:100,recipe:RECIPES.cheeseburger,built:['bottomBun'],selectedSource:null,selectedCooked:null,grill:[null,null],customerIndex:0,last:performance.now(),orderStarted:performance.now()};
const hits=[];
...
function newOrder(){const keys=Object.keys(RECIPES);state.recipe=RECIPES[keys[Math.floor(Math.random()*keys.length)]];state.built=['bottomBun'];state.selectedSource=null;state.selectedCooked=null;state.grill=[null,null];state.patience=100;state.orderStarted=performance.now();const c=CUSTOMERS[state.customerIndex++%CUSTOMERS.length];$('#customerName').textContent=c[0];$('#customerEmoji').textContent=c[1];$('#orderText').textContent=state.recipe.request;setHUD()}
...
function render(){const W=1600,H=420;ctx.clearRect(0,0,W,H);hits.length=0;ctx.fillStyle='#121413';ctx.fillRect(0,0,W,H);text('🔥 GRILL',28,34,26,'left');text('🍽️ ASSEMBLY',655,34,26,'left');
 // grill surface with two true slots
 ...
 // cold shelf
 roundRect(1405,55,175,335,15,'#b9bfbd','#666');drawBin('coldBun','BUNS','topBun',1420,75,145,88);drawBin('coldCheese','CHEESE','cheese',1420,175,145,88);drawBin('coldSauce','SAUCE','sauce',1420,275,145,88);
...
canvas.addEventListener('pointerdown',e=>{const h=pickHit(canvasPoint(e));if(!h)return;if(h.id==='sourcePatty'){state.selectedSource='patty';flash('Patty selected — tap a grill spot')}else if(h.id==='sourceBacon'){state.selectedSource='bacon';flash('Bacon selected — tap a grill spot')}else if(h.id==='grillSlot')chooseGrill(h.slotIndex);else if(h.id==='coldBun')addCold('topBun');else if(h.id==='coldCheese')addCold('cheese');else if(h.id==='coldSauce')addCold('sauce');else if(h.id==='assembly')addCooked()});
