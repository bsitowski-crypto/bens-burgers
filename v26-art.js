/* Ben's Burgers 26: one renderer for ingredient bins, order previews and assembled burgers. */
(function (root) {
'use strict';
let serial = 0;
const rises = {bottomBun:21, patty:25, cheese:6, bacon:8, lettuce:9, tomato:9, onion:6, pickles:6, sauce:4, topBun:0};
function defs(p) {
 return `<defs>
...
function layer(id,y,p,stage='ready') {
 const u=k=>`url(#${p+k})`;
 if(id==='bottomBun') return `<path d="M23 ${y-5}Q120 ${y-31} 217 ${y-5}L214 ${y+15}Q118 ${y+41} 26 ${y+15}Z" fill="${u('bun')}" stroke="#ae6226" stroke-width="2"/>${ellipse(120,y-5,97,23,u('bread'))}<path d="M34 ${y+12}Q120 ${y+34} 205 ${y+12}" fill="none" stroke="#f0b55b" stroke-width="3" opacity=".7"/>`;
 if(id==='topBun') {
...
function burger(layers,compact=false) {
 const p='bb'+(++serial);let y=213,body=ellipse(121,239,96,8,'#3d2314','opacity=".14"');
 for(const id of layers){body+=`<g data-layer="${id}">${layer(id,y,p)}</g>`;y-=rises[id]||0;}
 if(!layers.length)body=`<path d="M78 136q0-50 85 0M78 141h85M78 154h85M78 167q41 16 85 0" stroke="#9b713e" stroke-width="5" fill="none" opacity=".35"/>`;
 const top=compact?Math.max(0,y-55):30;return wrap(body,p,`0 ${top} 240 ${260-top}`,layers.length?'Burger with '+layers.join(', '):'Empty burger board');
}
function food(id,stage='ready') {
 ...
 const body=layer(item,70,p,stage);return wrap(body,p,'0 0 240 120',id);
}
...
const api={burger,food,drink,fries,machine,rises};
