...
class Kitchen extends api.Kitchen{
 ...
 advance(ms){
  const phase=this.phase,hold=!this.paused&&phase==='playing'&&Number.isFinite(ms)&&ms>0?Math.min(ms,this.active?.arrivalLeft||0):0;
  const machines=hold?[...this.grills.filter(Boolean),...Object.values(this.jobs).filter(Boolean)].map(j=>[j,j.elapsed]):[];
  super.advance(ms);
  // A restored READY patty must not burn during an entrance that blocks input.
  for(const [j,elapsed] of machines)j.elapsed=Math.min(j.duration||Infinity,elapsed+Math.max(0,ms-hold));
  if(phase!=='left'&&this.phase==='left')this.phaseLeft=2200;
 }
}
